...
MM.panels = {
	 ...
	 * @param {string} html The html to be inserted.
	 */
	htmlAppend: function(position, html) {
		MM.log("Appending code to panel " + position);
		// If we are in the center or right panel, we need to add a small br for preventing contents half displayed.
		if (position != "left") {
			html += "<br />";
		}
		$('#panel-' + position).append(html);
	...
	/**
	 * Prepend html to beginning of a panel
	 * @param {string} position The panel where insert the html.
	 * @param {string} html The html to be inserted.
	 */
	htmlPrepend: function(position, html) {
		MM.log("Appending code to panel " + position);
		// If we are in the center or right panel, we need to add a small br for preventing contents half displayed.
		if (position != "left") {
			html += "<br />";
		}
		$('#panel-' + position).prepend(html);
	 ...
	 */
	showOnly: function(position, content, settings) {
	...
	/**
	 * Append and shows a panel with some content
	 *
	 * @param {string} position The panel to be showed.
	 * @param {string} content The content to be added to the panel.
	 * @param {Object} settings Extra settings.
	 */
	appendShow: function(position, content, settings) {

		MM.panels.htmlAppend(position, content);
		MM.panels.currentPanel = position;
	...
	/**
	 * Prepend and shows a panel with some content
	 *
	 * @param {string} position The panel to be showed.
	 * @param {string} content The content to be added to the panel.
	 * @param {Object} settings Extra settings.
	 */
	prependShow: function(position, content, settings) {

		MM.panels.htmlPrepend(position, content);
		MM.panels.currentPanel = position;
