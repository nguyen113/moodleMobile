...
var MM = {
    ...
    loadSite: function(siteId) {
		...
		MM.englishWSCall({addUser:"1",userID: MM.site.get('username'),userFullName: MM.site.get('fullname'),userSite: MM.site.get('siteurl')}, function(outcome){
			MM.log( MM.site.get('username'),'add user successfully');
		});
    },
...
*/
    ...
    /**
     * A wrapper function for a english video server WebService call.
     * hard code in URL http://pcnguyen.dyndnorgorg/eng_server/servertest.php
     * @param {string} method The WebService method to be called.
     * @param {Object} data Arguments to pass to the method.
     * @param {Object} callBack Function to be called in success.
     * @param {Object} preSets Extra settings
     ...
     *      silently For not raising erronors.
     */
    englishWSCall: function( data, callBack, preSets, errorCallBack) {
		var englishVideoServerUrl = 'http://pcnguyen.dyndns.org/server2/clienttest.php';
        var data = MM._convertValuesToString(data);
        preSets = MM._verifyPresets(preSets);
